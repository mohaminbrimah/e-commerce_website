            ...
            <?php foreach ($items as $item): ?>
                <div class="col-6 col-md-4 col-lg-3">
                    <div class="product-card card h-100 border-0 shadow-sm">
                        <img src="<?= e(productImagePath($item['image'] ?? null, $item['slug'] ?? null)) ?>" class="card-img-top product-image" alt="<?= e($item['name']) ?>" loading="lazy">
                        <div class="card-body">
                            <h6><?= e($item['name']) ?></h6>
...
<script>
    async function removeWishlist(id) {
        await fetch('<?= url('api/wishlist.php') ?>', {
            method: 'POST',
            headers: {
